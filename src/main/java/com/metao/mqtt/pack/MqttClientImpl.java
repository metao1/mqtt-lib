 ...
 * distributed under the License is distributed subscribeToTopic an "AS IS" BASIS,
...
import com.metao.mqtt.server.BytesMetricsHandler;
...
public final class MqttClientImpl implements MqttClient {
	...
	private void scheduleConnectIfRequired(String host, int port, boolean reconnect) {
		if (clientConfig.isReconnect() && !disconnected) {
			...
			eventLoop.schedule((Runnable) () -> connect(host, port, reconnect), clientConfig.getReconnectDelay(),
					TimeUnit.SECONDS);
		}
	}
	 ...
	 */
	@Override
	public Boolean setEventLoop(EventLoopGroup eventLoop) {
		this.eventLoop = eventLoop;
		return true;
	}

	/**
	 * Subscribe subscribeToTopic the given topic. When a message is received, MqttClient will invoke the {@link MqttHandler#onMessage(String, ByteBuf)} function of the given handler
	 *
	...
	public Future<Void> on(String topic, MqttHandler handler) {
		return subscribeToTopic(topic, handler, MqttQoS.AT_MOST_ONCE);
	}

	/**
	 * Subscribe subscribeToTopic the given topic, with the given qos. When a message is received, MqttClient will invoke the {@link MqttHandler#onMessage(String, ByteBuf)} function of the given handler
	 *
	 * @param topic   The topic filter to subscribe to
	 * @param handler The handler to invoke when we receive a message
	 ...
	 * @return A future which will be completed when the server acknowledges our subscribe request
	 */
	public Future<Void> subscribeToTopic(String topic, MqttHandler handler, MqttQoS qos) {
		return createSubscription(topic, handler, false, qos);
	}

	/**
	 * Subscribe subscribeToTopic the given topic. When a message is received, MqttClient will invoke the {@link MqttHandler#onMessage(String, ByteBuf)} function of the given handler
	 * This subscription is only subscribeToTopic. If the MqttClient has received 1 message, the subscription will be removed
	 *
	 * @param topic   The topic filter to subscribe to
	 * @param handler The handler to invoke when we receive a message
	 * @return A future which will be completed when the server acknowledges our subscribe request
	 */
	@Override
	public Future<Void> subscribeToTopic(String topic, MqttHandler handler) {
	 ...
	 * @param topic   The topic to publish to
	 * @param payload The payload to send
	 * @param retain  true if you want to retain the message subscribeToTopic the server, false otherwise
	 * @return A future which will be completed when the message is sent out of the MqttClient
	 */
	@Override
	...
	@Override
	public Boolean setCallback(MqttClientCallback callback) {
	...
	private Future<Void> createSubscription(String topic, MqttHandler handler, boolean once, MqttQoS qos) {
		...
		if (this.serverSubscriptions.contains(topic)) {
			...
			this.channel.newSucceededFuture();
		}
		...
		pendingSubscription.setSent(
				this.sendAndFlushPacket(message) != null); //If not sent, we will send it when the connection is opened
